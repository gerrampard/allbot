...
| `trigger_handler.py` | Trigger | 497 | TriggerHandler: 消息去重、路由构建、触发词匹配、管理员检测、用户文本提取、pending run 管理 |
...
| `reply_writer.py` | Reply | 963 | ReplyWriter: 回复分片发送、流式 delta 累积节流、终态收敛、pending run 生命周期、模型错误分类与自动重试 |
| `event_handler.py` | Events | 205 | EventHandler: 网关事件分发（agent/chat/health/shutdown/node.pair/device.pair/cron 等）、pending run 终态收敛、事件转发到 to-wxids |
